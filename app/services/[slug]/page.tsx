...
const services = {
  ardoise: {
    ...
    beforeAfter: {
      ...
      after: "https://images.unsplash.com/photo-1591955506264-3f5a6834570a?q=80&w=800&auto=format&fit=crop",
...
const otherServices = (currentSlug: string) =>
  Object.entries(services).filter(([slug]) => slug !== currentSlug);
...
export async function generateMetadata({
  ...
  return {
    ...
    description: `Corrignan Couverture pose ${service.article} sur Lorient, Lanester, Auray et tout le bassin lorientais. Devis gratuit sous 48h. ${service.tagline}.`,
  };
}
    ...
    <main className="bg-night-800">
      ...
      <section className="relative overflow-hidden">
        <div className="relative w-full h-64 md:h-96">
          <Image src={service.image} alt={service.imageAlt} fill className="object-cover" priority />
        </div>
        <div className="absolute inset-0 bg-night-900/60 flex flex-col justify-end p-6 md:p-12">
          <Link
            href="/services"
            className="inline-flex items-center gap-1.5 text-white/70 hover:text-white text-sm mb-4 transition w-fit"
          >
          ...
          <p className="text-white/70 text-sm mb-1">{service.tagline}</p>
          <h1 className="text-4xl md:text-5xl font-extrabold text-white">{service.label}</h1>
        </div>
      </section>

      {/* ── Description + Points forts ── */}
      <section className="py-12 px-4 mx-auto max-w-screen-xl lg:py-16 lg:px-6 bg-night-800">
        <div className="lg:grid lg:grid-cols-2 lg:gap-16 items-start">
          <div>
            <h2 className="text-2xl font-bold text-white mb-4">
              Pourquoi choisir {service.article} ?
            </h2>
            <p className="text-[#b8b09a] leading-relaxed mb-8">
              {service.description}
            </p>
            <Link
              href="/contact"
              className="inline-flex items-center gap-2 text-night-900 bg-gradient-to-br from-gold-600 to-gold-400 hover:opacity-90 transition font-medium rounded-lg text-sm px-5 py-2.5"
            >
          ...
          <div className="bg-night-700 rounded-2xl p-6 md:p-8 mt-10 lg:mt-0 border border-[#c9a84c33]">
            <h3 className="text-lg font-bold text-white mb-5">Les points forts</h3>
            <ul className="space-y-3">
              {service.points.map((point, i) => (
                <li key={i} className="flex items-start gap-3">
                  <span className="mt-0.5 w-5 h-5 rounded-full bg-gold-600 flex items-center justify-center shrink-0">
                    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
                      <path d="M20 6L9 17l-5-5" />
                    </svg>
                  </span>
                  <span className="text-sm text-[#b8b09a] leading-relaxed">{point}</span>
                </li>
              ))}
            </ul>
          </div>
      ...
      {/* ── 2e bloc description + photo ── */}
      <section className="bg-night-800 py-12 px-4 lg:py-16 lg:px-6">
        <div className="mx-auto max-w-screen-xl gap-12 items-center lg:grid lg:grid-cols-2 lg:gap-16">
          <div className="relative w-full h-64 md:h-80 rounded-2xl overflow-hidden">
            <Image src={service.description2.image} alt={service.description2.imageAlt} fill className="object-cover" />
          </div>
          <div className="mt-8 lg:mt-0">
            <h2 className="text-2xl font-bold text-white mb-4">
              {service.description2.title}
            </h2>
            <p className="text-[#b8b09a] leading-relaxed">
              {service.description2.body}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 px-4 mx-auto max-w-screen-xl lg:py-16 lg:px-6 bg-night-800">
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-white">Avant / Après</h2>
          <p className="mt-1 text-sm text-[#b8b09a]">{service.beforeAfter.caption}</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <p className="text-xs font-semibold text-[#b8b09a] uppercase tracking-widest mb-2">Avant</p>
            <div className="relative w-full h-56 md:h-72 rounded-2xl overflow-hidden">
              <Image src={service.beforeAfter.before} alt="Avant travaux" fill className="object-cover" />
            </div>
          </div>
          <div>
            <p className="text-xs font-semibold text-[#f0d080] uppercase tracking-widest mb-2">Après</p>
            <div className="relative w-full h-56 md:h-72 rounded-2xl overflow-hidden">
              <Image src={service.beforeAfter.after} alt="Après travaux" fill className="object-cover" />
            </div>
          </div>
        </div>
      </section>

      {/* ── Témoignages filtrés par service ── */}
      <TestimonialsSwiper />

      {/* ── FAQ spécifique au service ── */}
      <FAQ
        faqs={service.faqs}
        title={`Questions fréquentes sur ${service.article}`}
        subtitle={`Tout ce que vous voulez savoir avant de nous confier votre projet de ${service.label.toLowerCase()}, on se déplace sur Lorient, Lanester, Auray et tout le bassin lorientais.`}
      />
      ...
      {/* ── CTA ── */}
      <section className="bg-night-800 py-12 px-4 mx-auto max-w-screen-sm text-center lg:py-16">
        <h2 className="text-3xl font-extrabold text-white mb-4">
          Un projet en {service.article} ?
        </h2>
        <p className="text-[#b8b09a] mb-6">
          On se déplace gratuitement pour évaluer votre chantier et vous remettre un devis clair sous 48h.
        </p>
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
          <Link
            href="/contact"
            className="text-night-900 bg-gradient-to-br from-gold-600 to-gold-400 hover:opacity-90 transition font-medium rounded-lg text-sm px-5 py-2.5"
          >
            Demander un devis gratuit
          </Link>
          <a
            href="tel:0297000000"
            className="flex items-center gap-2 text-sm font-medium text-night-800 hover:text-night-900 transition px-5 py-2.5 rounded-lg border border-gold-200"
          >
